...
import { Upload, FileText, AlertCircle, ChevronDown, ChevronUp, X, CheckCircle } from "lucide-react"
...
export default function UploadPage() {
  ...
  const [validRows, setValidRows] = useState<DonorRow[]>([])
  ...
  const handleParse = async () => {
    ...
    try {
      ...
      const res = await instance.post('/upload/parse', formData, {
        headers: { 'Content-Type': 'multipart/form-data' },
      })

      const { validRows: vRows, invalidRows, summary: sum } = res.data.data
      ...
      setBadRows(
        invalidRows.map((r: any) => ({
          rowNumber: r.row,
          data: r.data,
          reason: Array.isArray(r.reasons) ? r.reasons.join(', ') : r.reasons,
        }))
      )
      setSummary(sum)
      setStage("preview")
  ...
  const handleCommit = async () => {
    ...
    try {
      await instance.post('/upload/commit', { validRows })
      setStage("success")
    } catch (err: any) {
      setError(err?.message || "Failed to commit data. Please try again.")
    } finally {
      setIsLoading(false)
    }
  }
      ...
      <div className="p-6 md:p-8 max-w-4xl mx-auto">
        {stage === "success" ? (
          <div className="flex flex-col items-center justify-center py-20 text-center space-y-4">
            ...
            <h2 className="text-2xl font-semibold text-foreground">Data committed successfully</h2>
            <p className="text-muted-foreground">
              {summary.valid} donor record{summary.valid !== 1 ? "s" : ""} have been saved.
            </p>
            ...
            <div className="bg-card rounded-lg border border-border p-4">
              <div className="flex items-center gap-6 text-sm">
                <span className="text-muted-foreground">
                  <span className="font-semibold text-foreground">{summary.total}</span> rows found
                </span>
                <span className="text-muted-foreground">
                  <span className="font-semibold text-success">{summary.valid}</span> valid
                </span>
                <span className="text-muted-foreground">
                  <span className="font-semibold text-destructive">{summary.invalid}</span> rows flagged
                </span>
              </div>
            </div>
