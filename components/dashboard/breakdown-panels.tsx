...
type BreakdownType = "segment" | "campaign" | "channel"

interface BreakdownPanelsProps {
  isEmpty?: boolean
}

export function BreakdownPanels({ isEmpty }: BreakdownPanelsProps) {
  ...
  const TabSelector = ({
    ...
    <div className="flex items-center gap-1 rounded-lg bg-muted p-1">
      {(["segment", "campaign", "channel"] as const)
        .filter((t) => t !== exclude)
        .map((type) => (
          <button
            key={type}
  ...
  const renderPanelContent = (type: BreakdownType) => {
    ...
    switch (type) {
      ...
      case "campaign":
        return renderHorizontalBarChart(campaignData)
      case "channel":
        return renderHorizontalBarChart(channelData)
    }
  }
